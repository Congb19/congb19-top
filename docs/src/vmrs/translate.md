...
随后，我们在此基础上提出了一个互动式的推荐系统，该系统以讲故事的方式推荐电影，以促进用户与推荐系统之间的交流。我们使用LSM模型来生成推荐故事，通过为电影分配合适的演员、角色和叙事结构来描述一组推荐电影。推荐故事是自动构建以吸引用户的注意力的，且支持多层次的可视化和动画效果，并可在交互推荐过程中灵活调整。与以前的在线推荐系统不同，我们的方法揭示了通常对用户隐藏的信息，并允许对电影相似性进行交互式探索。我们还提供了几个案例来说明LSM和交互式推荐在不同的推荐任务中的应用。
