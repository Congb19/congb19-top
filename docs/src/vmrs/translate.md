...
面向普通用户的交互式推荐方法由两个部分组成：用于提取个人电影偏好的LSM（第3节）和带有StoryTelling的交互式推荐（第4节）。在本节中，我们首先介绍协作过滤算法中的la-tent空间。然后我们描述了我们的推荐方法和推荐度的度量。最后，本文提出了一种基于语义概念集将高维数据统计转化为推荐域的LSM方法。LSM也用于

在第4节中设计推荐讲故事和用户交互。在这项工作中，LSM和交互式推荐系统的设计是同时进行的，以确保同一套语义概念可以用于电影的交互式推荐和探索。
