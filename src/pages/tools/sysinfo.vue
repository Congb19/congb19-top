<template>
  <h3>System:</h3>
  <n-li v-for="item in info.system">{{ item.label + ': ' + item.value }}</n-li>
  <h3>Browser:</h3>
  <n-li v-for="item in info.browser">{{ item.label + ': ' + item.value }}</n-li>
  <h3>Help:</h3>
  <n-li>分辨率为逻辑像素，即 分辨率=屏幕分辨率/总缩放倍率；</n-li>
  <n-li>总缩放倍率=系统缩放倍率×浏览器缩放倍率（例如，系统125%，浏览器100%，则总1.25）；</n-li>
</template>
